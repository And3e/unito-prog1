...
bool str_ugualiR(const char *pStrA, const char *pStrB) {
  if ((*pStrA == '\0' && *pStrA != *pStrB) ||
      (*pStrB == '\0' && *pStrB != *pStrA) || (*pStrA != *pStrB)) {
    return false;
  }

  if (*pStrA == *pStrB && *pStrA + 1 == '\0' && *pStrB + 1 == '\0') {
    return true;
  }
...
int main(void) {
#define NUM_STR 7
  const char *stringhe[NUM_STR] = {"Bora Bora",     "Barcellona", "Santorini",
                                   "Havana",        "Chiang Mai", "Reykjavik",
                                   "Rio de Janeiro"};
